                ...
                <form method="POST" action="{{ route('adminStoreCustomerProfileDet') }}">
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">                      
                        ...
                        <label class="block">
                            ...
                            <span class="relative mt-1.5 flex">
                            <select
                                class="form-input peer w-full rounded-full border border-slate-300 bg-transparent px-3 py-2 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent"
                                name="country"
                                required
                            >
                            <option value="" selected disabled>Select Country</option>
                            @foreach ($countries as $country)
                                <option value="{{ $country }}" {{$data->country!=null && $data->country==$country ? "selected" : ""}}>{{ $country }}</option>
                            @endforeach
                            </select>
                            </span>
                        </label>
