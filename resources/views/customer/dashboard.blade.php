...
@section('content')
    <div class="mt-4 grid grid-cols-12 gap-4 sm:mt-5 sm:gap-5 lg:mt-6 lg:gap-6">
        <div class="col-span-12">
            <div class="card p-0">
                <div class="">
                  <div x-init="$nextTick(()=>$el._x_swiper = new Swiper($el, { effect: 'coverflow', coverflowEffect: {rotate: 35, slideShadows: false,}, pagination: { el: '.swiper-pagination',clickable: true}}))"
        ...
        <div class="col-span-12 lg:col-span-8">
            <div :class="$store.breakpoints.smAndUp &amp;&amp; 'via-purple-300'" class="card mt-12 bg-gradient-to-l from-pink-300 to-indigo-400 p-5 sm:mt-0 sm:flex-row via-purple-300">
              ...
              <div class="mt-2 flex-1 pt-2 text-center text-white sm:mt-0 sm:text-left">
                ...
                <p class="mt-2 leading-relaxed">
                  Last Login On: 
                  <span class="font-semibold text-slate-100">{{ date('d M Y h:m:s', strtotime(Auth::user()->last_login_datetime)) }}</span>
                </p>
              </div>
            </div>            
        </div>
        ...
        <div class="col-span-12 grid grid-cols-1 gap-4 sm:gap-5 lg:col-span-4 lg:gap-6">
            <div class="card px-4 py-4 sm:px-5">
                <div class="pb-3 flex h-8 items-center justify-between">
                    <h2 class="font-medium tracking-wide text-xl text-success dark:text-navy-100">
                    Your Incomes
                    </h2>
                </div>
                <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-3 sm:gap-5 lg:gap-6">
                <div class="card bg-gradient-to-r from-blue-500 to-indigo-600 px-5 pb-5">
                    <div>
                        ...
                        <span class="text-white">${{ Auth::user()->fund_wallet_amount }}</span>
                        </p>
                    </div>
                ...
                <div class="grid grid-cols-1 gap-4 sm:col-span-2 sm:grid-cols-2 sm:gap-5 lg:gap-6">                    
                    <div class="relative flex flex-col overflow-hidden rounded-lg bg-gradient-to-br from-pink-500 to-rose-500 p-3.5">
                        ...
                        <p class="mt-4 text-2xl font-medium text-white">${{ Auth::user()->roi_wallet_amount }}</p>
                    ...
                    <div class="relative flex flex-col overflow-hidden rounded-lg bg-gradient-to-br from-amber-400 to-orange-600 p-3.5">
                        ...
                        <div class="flex items-end justify-between space-x-2">
                        <p class="mt-4 text-2xl font-medium text-white">${{ Auth::user()->working_wallet_amount }}</p>
        ...
        <div class="col-span-12 lg:col-span-8">
            <div class="card pb-1">
                ...
                <script type="text/javascript" src="https://files.coinmarketcap.com/static/widget/coinPriceBlock.js"></script><div id="coinmarketcap-widget-coin-price-block" coins="1,1027,825,74,2,1958,5426" currency="USD" theme="light" transparent="false" show-symbol-logo="true"></div>
                </div>
            </div>
        </div>

        <div class="col-span-12 grid grid-cols-1 gap-4 sm:gap-5 lg:col-span-4 lg:gap-6">            
            <div class="card bg-info/15 p-4 dark:bg-transparent justify-center p-4.5">
            ...
            <div class="card bg-secondary/15 p-4 dark:bg-transparent justify-center p-4.5">
                <div class="flex items-center justify-between">
                    <div>
                        <p
                        class="text-base font-semibold text-slate-700 dark:text-navy-100"
                    ...
                    <div class="grid grid-cols-1 gap-4">
                        <div>
                            ...
                            <div class="flex items-center space-x-2 pb-2">
                                <input type="text" class="text-info w-full" name="copyLinkLeft" id="copyLinkLeft" value="{{ config('app.url')."/customer/direct/register?userID=".Auth::user()->login_id."&position=Left" }}" />
                            </div>
                            ...
                            {{-- <a target="_blank" class="btn bg-success px-3 py-1 text-xs font-medium text-white hover:bg-success-focus focus:bg-success-focus active:bg-success-focus/90" href="https://api.whatsapp.com/send?phone&text=Use my referral link to join {{ config('app.name') }} Platform. {{ config('app.url') }}/customer/direct/register?userID={{ Auth::user()->login_id }}&position=Left"><p>WhatsApp Share</p></a> --}}
                        </div>
                        <div>
                            <h3 class="font-bold text-black">Right Link</h3>
                            <div class="flex items-center space-x-2 pb-2">
                                <input type="text" class="text-info w-full" name="copyLinkRight" id="copyLinkRight" value="{{ config('app.url')."/customer/direct/register?userID=".Auth::user()->login_id."&position=Right" }}" />
                            </div>
                            ...
                            {{-- <a target="_blank" class="btn bg-success px-3 py-1 text-xs font-medium text-white hover:bg-success-focus focus:bg-success-focus active:bg-success-focus/90" href="https://api.whatsapp.com/send?phone&text=Use my referral link to join {{ config('app.name') }} Platform. {{ config('app.url') }}/customer/direct/register?userID={{ Auth::user()->login_id }}&position=Right"><p>WhatsApp Share</p></a> --}}
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="col-span-12 grid grid-cols-1 gap-4 sm:gap-5 lg:col-span-4 lg:gap-6">
            <div class="card bg-gradient-to-r from-blue-500 to-indigo-600 px-5 pb-5">
                <div>
                    ...
                    <p class="mt-4 font-inter text-2xl font-semibold">
                    <span class="text-white">${{ $totalInvestment }}</span>
                    </p>
                </div>
                <div class="absolute bottom-0 right-0 overflow-hidden rounded-lg">
                    <img
