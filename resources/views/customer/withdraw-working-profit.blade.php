    ...
    <div>
        <div class="py-5 lg:py-6 flex justify-between">
          ...
          <div class="space-y-2 md:space-y-2">
            <a href="{{ route('customerWithdrawWorkingProfitReport') }}" class="btn bg-primary px-3 py-1 text-xs font-medium text-white hover:bg-primary-focus focus:bg-primary-focus active:bg-primary-focus/90 mr-2">
              WORKING PROFIT REPORT
            </a>
          </div>
        </div>
          ...
          <div class="card px-4 pb-4 sm:px-5">
            ...
            <form method="POST" action="{{ route('customerWithdrawWorkingProfitSave') }}">
                ...
                <div class="max-w-xl">
                    ...
                    @if (config('services.withdrawalcharges.working') > 0)
                        <h4 class="py-4 text-warning">Withdrawal charge: {{ config('services.withdrawalcharges.working') }}%</h4>
                    @endif
